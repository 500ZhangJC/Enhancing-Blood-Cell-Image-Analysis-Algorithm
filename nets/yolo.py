...
class SPPCSPC(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=(5, 9, 13)):
        ...
        self.cv1 = Conv(c1, c_, 1, 1, act='mish')  # Add activation
        self.cv2 = Conv(c1, c_, 1, 1, act='mish')  # Add activation
        self.cv3 = Conv(c_, c_, 3, 1, act='mish')  # Add activation
        self.cv4 = Conv(c_, c_, 1, 1, act='mish')  # Add activation
        self.cv5 = Conv(4 * c_, c_, 1, 1, act='mish')  # Add activation
        self.cv6 = Conv(c_, c_, 3, 1, act='mish')  # Add activation
        self.cv7 = Conv(2 * c_, c2, 1, 1)  # Output channels c2
...
class YoloBody(nn.Module):
    def __init__(self, anchors_mask, num_classes, phi, pretrained=False):
        ...
        # Define Multi-Path Residual Module
        self.multi_path_residual = MultiPathResidualBlock(transition_channels * 4, transition_channels * 4, e=e, n=n)
    ...
    def forward(self, x):
        ...
        # Apply Multi-Path Residual Module
        P3 = self.multi_path_residual(P3)
