...
class Conv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=nn.GELU()):  # ch_in, ch_out, kernel, stride, padding, groups
...
class ISOmodule(nn.Module):
    ...
    def forward(self, x):
        ...
        return x * attn
class IPOAtt(nn.Module):
    def __init__(self, d_model):
        super().__init__()

        self.proj_1 = nn.Conv2d(d_model, d_model, 1)
        self.activation = nn.GELU()
        self.spatial_gating_unit = ISOmodule(d_model)
...
class Backbone(nn.Module):
    def __init__(self, transition_channels, block_channels, n, phi, pretrained=False):
        ...
        self.dark5 = nn.Sequential(
            ...
            ISOmodule(transition_channels * 32),  # 添加IPO模块
        )
